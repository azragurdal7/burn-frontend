...
const AIConsult = () => {
    ...
    useEffect(() => {
        const loadImageAndPatientData = async () => {
            ...
            if (patient.photoPath) {
                try {
                    const photoUrl = `${API_BASE_URL}/${patient.photoPath.replace(/\\/g, '/')}`;
            ...
            try {
                const patientResponse = await fetch(`${API_BASE_URL}/api/Patient/${patient.patientID}`);
    ...
    const handleSendToAI = async () => {
        ...
        try {
            ...
            const aiResponse = await fetch(`${FLASK_AI_API_URL}/predict`, {
                method: 'POST',
                body: formData
            });
            ...
            const backendResponse = await fetch(`${API_BASE_URL}/api/Patient/update-ai-analysis/${patient.patientID}`, {
                method: 'POST',
    ...
    const confirmVerification = async () => {
        try {
           const response = await fetch(`${API_BASE_URL}/api/Patient/update-verified/${patient.patientID}`, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
...
const styles = {
    container: { display: 'flex', flexDirection: 'column', backgroundColor: "#ecf0f1", alignItems: 'center', padding: '20px', fontFamily: 'Arial, sans-serif' },
    header: { width: '100%', display: 'flex', justifyContent: 'flex-start', marginBottom: '20px' },
    backButton: { position: "absolute",padding: '10px 15px', top:"10px", left:"30px", width: "100px",backgroundColor: '#7ba9db', color: 'white', border: 'none', borderRadius: '6px', cursor: 'pointer', fontSize: '15px' },
    contentContainer: { display: 'flex', flexDirection: 'row', width: '100%', maxWidth: '1000px', gap: '20px' },
    imageContainer: { flex: 1, backgroundColor: "#ecf0f1", boxShadow:"0 4px 20px rgba(0, 0, 0, 0.8)", border: '1px solid #ddd', padding: '10px', borderRadius: '8px', display: 'flex', justifyContent: 'center', alignItems: 'center', minHeight: '300px' },
    image: { maxWidth: '100%', maxHeight: '400px', height: 'auto', borderRadius: '4px' },
    resultsContainer: { flex: 1,backgroundColor: "#ecf0f1", boxShadow:"0 4px 20px rgba(0, 0, 0, 0.8)", border: '1px solid #ddd', padding: '20px', borderRadius: '8px', backgroundColor: '#f8f9fa' },
    ...
    confirmButton: { padding: '10px 18px', margin: '5px', backgroundColor: '#7ba9db', color: 'white', border: 'none', borderRadius: '5px', cursor: 'pointer', fontSize: '14px' },
